...
def has_annual_report_intel(company: dict[str, Any]) -> bool:
    if REPORTS_BY_SLUG.get(str(company.get("slug") or "")):
        return True
...
def latest_annual_report_year(company: dict[str, Any], snapshot_year: int) -> int:
    extracted = REPORTS_BY_SLUG.get(str(company.get("slug") or ""), [])
    if extracted:
        return max(int(item["report_year"]) for item in extracted if item.get("report_year"))
...
def annual_report_intel_snapshot(company: dict[str, Any]) -> dict[str, Any]:
    reports = sorted(REPORTS_BY_SLUG.get(str(company.get("slug") or ""), []), key=lambda item: item.get("report_year", 0))
    if reports:
        latest = reports[-1]
        previous = reports[-2] if len(reports) > 1 else {}
        fields = {"ai": "ai_mentions", "cloud": "cloud_mentions", "dataCenter": "data_center_mentions", "regulation": "regulation_mentions", "competition": "competition_mentions", "marginPressure": "margin_pressure_mentions", "restructuring": "restructuring_mentions"}
        growth = sum(float(latest.get(fields[key], 0)) - float(previous.get(fields[key], latest.get(fields[key], 0))) for key in ("ai", "cloud", "dataCenter"))
        pressure = sum(float(latest.get(fields[key], 0)) - float(previous.get(fields[key], latest.get(fields[key], 0))) for key in ("regulation", "competition", "marginPressure", "restructuring"))
        return {"keywordTrend": growth - pressure * .35, "healthKeywordBalance": growth - max(0, pressure)}
...
def growth_components(company: dict[str, Any], snapshot_year: int) -> dict[str, Any]:
    ...
    sentiment = SENTIMENT_BY_SLUG.get(str(company.get("slug") or ""), {}).get("sentiment", {}).get("current")
    sentiment_lift = float(sentiment) * .12 if isinstance(sentiment, (int, float)) else 0
...
def main() -> int:
    ...
    for report in report_history.get("reports", []):
        if report.get("report_data_status") == "extracted" and report.get("company_slug"):
